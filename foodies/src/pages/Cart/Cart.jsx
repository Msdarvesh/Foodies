...
const Cart = () => {
  ...
  const { foodList, incrementQuantity, decrementQuantity, quantities,removeFromCart } = useContext(StoreContext);
      ...
      <div className="row">
        <div className="col-lg-8">
          ...
          {/* {cartItems.length === 0 ? (
            ...
            cartItems.map(food => (
              <div className="card mb-4 cart-item" key={food.id}>
                <div className="row g-0">
                  <div className="col-md-4">
                    <img src={food.image} className="img-fluid rounded-start" alt={food.name} />
                  </div>
                  <div className="col-md-8">
                    <div className="card-body">
                      ...
                      <div className="d-flex align-items-center">
                        <button className="btn btn-outline-secondary btn-sm me-2" onClick={() => decrementQuantity(food.id)}>-</button>
                        <span>{quantities[food.id]}</span>
                        <button className="btn btn-outline-secondary btn-sm ms-2" onClick={() => incrementQuantity(food.id)}>+</button>
                      </div>
                      <p className="card-text mt-2"><small className="text-muted">Total: ${(food.price * quantities[food.id]).toFixed(2)}</small></p>
                    </div>
                  </div>
                </div>
              </div>
                  ...
                  <div key={food.id} className="row cart-item mb-3">
                    ...
                    <div className="col-md-2">
                      <div className="input-group">
                        <button className="btn btn-outline-secondary btn-sm" onClick={() => decrementQuantity(food.id)}>-</button>
                        <input
                          ...
                          readOnly
                        />
                        <button className="btn btn-outline-secondary btn-sm" onClick={() => incrementQuantity(food.id)}>+</button>
                      </div>
                    </div>
